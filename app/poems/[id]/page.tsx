    ...
    <main className="min-h-screen bg-black text-white">
      ...
      <div className="fixed top-0 left-0 right-0 z-50 bg-black/80 backdrop-blur-sm border-b border-white/10">
        <div className="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center">
          <Link href="/poems" className="text-white/40 hover:text-white transition-colors">
            ← All Poems
          </Link>
          <div className="flex gap-6">
            {prevPoem && (
              <Link href={`/poems/${prevPoem.id}`} className="text-white/40 hover:text-white transition-colors">
                ← Previous
              </Link>
            )}
            {nextPoem && (
              <Link href={`/poems/${nextPoem.id}`} className="text-white/40 hover:text-white transition-colors">
                Next →
              </Link>
            )}
          </div>
...
{poem.text}
          ...
          <div className="mt-16 pt-8 border-t border-white/10 flex justify-between">
            ...
            {nextPoem ? (
              <Link href={`/poems/${nextPoem.id}`} className="text-white/40 hover:text-white transition-colors text-right">
